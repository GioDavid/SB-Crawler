...
describe("fetchHackerNewsHtml", () => {
  ...
  it("returns HTML for a successful response", async () => {
    ...
    await expect(fetchHackerNewsHtml()).resolves.toBe(html);
  ...
  it("throws when Hacker News returns HTTP 404", async () => {
    ...
    await expect(fetchHackerNewsHtml()).rejects.toThrow(
  ...
  it("propagates network errors", async () => {
    ...
    await expect(fetchHackerNewsHtml()).rejects.toThrow("Network error");
  });
});
