...
export function parseHackerNewsEntries(html: string): HackerNewsEntry[] {
    ...
    .each((_, element) => {
      ...
      const commentsText = subtextRow
        ...
        .filter((_, link) => {
          ...
          return text.includes("comment") || text === "discuss";
        })
        .last()
        .text()
        .trim();

      const number = Number.parseInt(numberText, 10) || 0;

      const points = Number.parseInt(pointsText, 10) || 0;

      const comments = Number.parseInt(commentsText, 10) || 0;
